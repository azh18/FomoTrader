...
format1_dir = '../data/data_format1_20181007_20181014.h5'
format2_dir = '../data/data_format2_20181007_20181014.h5'
...
def extract_features_from_memory(memory_obj):
    ...
    for i in range(0, len(data_time_interval)):
        ...
        for j in range(0, 1):  # ticker_num
            v_macd = memory_obj.timed_macds[i][j][-n_slot:]
            v_rsi = memory_obj.timed_rsi[i][j][-n_slot:]
            v_hks = memory_obj.timed_stochks[i][j][-n_slot:]
            v_hds = memory_obj.timed_stochds[i][j][-n_slot:]
            v_lbs = memory_obj.timed_lowerbs[i][j][-n_slot:]
            v_hbs = memory_obj.timed_higherbs[i][j][-n_slot:]
            v_mbs = memory_obj.timed_middlebs[i][j][-n_slot:]
            # v_price = ???
            v1 = np.concatenate((v_macd, v_rsi, v_hks, v_hds, v_lbs, v_hbs, v_mbs))
...
def handle_bar(counter,  # a counter for number of minute bars that have already been tested
               ...
               price_future, # price in future two days
               ):
    if counter == 0:
        memory.records = None
    ...
    if counter > 4500:
        ...
        #t1 = timer.time()
        labels = []
        price_1min_future = get_avg_price(price_future[0])
        price_2min_future = get_avg_price(price_future[1])
        labels = price_2min_future - price_1min_future
...
class backTest:
    ...
    def backTest(self):
        ...
        for i in range(len(keys)-7):
            data_cur_min = format2[keys[i]][:]
            data_fur_1min = format2[keys[i + 1]][:]
            data_fur_2min = format2[keys[i + 7]][:]
            # Update position and memory
            [_, self.memory] = handle_bar(i,
                                          keys[i],
                                          data_cur_min,
                                          self.memory,
                                          (data_fur_1min, data_fur_2min))

            # Update position and timer
            if '09:30:00' in keys[i]:
                print(keys[i][:10])
        self.memory.records.to_csv("fea_labels.csv")
        format1.close()
        format2.close()
        return
